...
export function* readOrderAtIndex(index, storeInstance, products) {
  const [buyer, productIndex, price, message] = yield call(
    storeInstance.orders.call,
    index
  );
...
export function* readProductAtIndex(storeInstance, index) {
  const [name, available, price, ratingsSum, nRatings] = yield call(
    storeInstance.products.call,
    index
  );
...
export function* loadMall() {
  const balance = yield cps(
    Web3Service.web3.eth.getBalance,
    Web3Service.mallInstance.address
  );
...
export function* initializeCampaign({ cname = "" }) {
  const address = yield select(state => state.root.address);

  const txOptions = {
    from: address,
    value: Web3Service.web3.toWei(1, "ether")
  };
  const resp = yield call(
    Web3Service.smartAdInstance.initializeCampaign.sendTransaction,
    name,
    txOptions
  );
  console.log(resp);
...
export function* onWindowLoad() {
  ...
  if (typeof window.web3 !== "undefined") {
    yield call(
      [Web3Service, Web3Service.setProvider],
